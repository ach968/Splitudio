...
export default function Piano({
  ...
  if (playAlong == true && playAlongBuffer) {
    upcomingMidis = new Set(
      Array.from(playAlongBuffer.keys()).map((encodedNote) =>
        Number(encodedNote.split("-")[3])
      )
    );
    playingMidis = new Set(
      Array.from(playAlongBuffer.keys())
        .filter((key) => playAlongBuffer.get(key) === true)
        .map((encodedNote) => Number(encodedNote.split("-")[3]))
    );
  }
